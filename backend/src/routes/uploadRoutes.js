...
// ── Use Disk Storage to prevent memory exhaustion with 1GB files ──────────────
...
router.post('/', protect, upload.single('file'), (req, res) => {
  ...
  cloudinary.uploader.upload(
    ...
    (error, result) => {
      // ── CLEANUP: Always remove local file after processing ─────────────────
      fs.unlink(req.file.path, (err) => {
        if (err) console.error('Failed to cleanup temp upload:', err);
      });
