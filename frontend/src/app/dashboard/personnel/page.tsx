    ...
    return (
      <div className="h-[70vh] flex items-center justify-center">
        <div className="bg-white rounded-3xl p-12 shadow-2xl text-center max-w-lg border border-zinc-100">
          ...
          <h2 className="text-3xl font-bold text-zinc-900 tracking-tight mb-4 uppercase">Security Alert</h2>
    ...
    <div className="space-y-12 pb-24">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-10">
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] font-bold uppercase tracking-[0.4em] text-zinc-400">Governance Protocol</span>
          </div>
          <motion.h1 
            initial={{ y: 20, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            className="text-4xl md:text-5xl font-bold tracking-tighter text-zinc-950 uppercase italic"
          >
            Personnel <span className="text-zinc-200 not-italic font-light">Inventory</span>
          </motion.h1>
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2 px-3 py-1.5 bg-emerald-50 text-emerald-600 text-[9px] font-bold uppercase tracking-widest rounded-full border border-emerald-100 shadow-sm shadow-emerald-500/5">
        ...
        <div className="relative group w-full lg:w-[500px]">
          <input 
            ...
            className="bg-white border border-zinc-100 rounded-3xl px-10 py-6 text-xs font-bold uppercase tracking-widest w-full focus:outline-none focus:ring-8 focus:ring-black/[0.02] focus:border-zinc-950 transition-all shadow-sm"
          />
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-8 bg-zinc-50/50 p-3 rounded-[3rem] border border-zinc-100 shadow-sm">
        <div className="flex flex-wrap items-center gap-2">
          {[
            ...
            <button
              ...
              className={`flex items-center gap-4 px-8 py-4 text-[10px] font-bold uppercase tracking-[0.2em] transition-all rounded-2xl relative overflow-hidden group ${
                tab === t.id 
        ...
        <div className="px-8 py-3 bg-white rounded-full border border-zinc-100 shadow-sm">
          <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Node_Count: <span className="text-zinc-950">{nodes.length}</span></span>
        </div>
      </div>
        ...
        <AnimatePresence mode="wait">
          {loading ? (
            ...
            >
              ...
              <p className="text-[11px] font-bold uppercase tracking-[0.6em] text-zinc-300 italic">Re-aligning Neural Nodes...</p>
            </motion.div>
          ) : (
            <motion.div
              key={tab}
            ...
            >
              {filteredNodes.length > 0 ? (
                filteredNodes.map((node, i) => (
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between mb-10 relative z-10">
                      <div className="w-20 h-20 bg-zinc-50 border border-zinc-100 text-zinc-950 flex items-center justify-center text-4xl font-bold rounded-2xl shadow-sm group-hover:bg-zinc-950 group-hover:text-white group-hover:rotate-6 transition-all duration-500">
                    ...
                    <div className="space-y-2 mb-10 relative z-10">
                      <h3 className="text-2xl font-bold text-zinc-950 tracking-tighter uppercase italic group-hover:text-zinc-950 transition-colors">{node.name}</h3>
                    ...
                    <div className="flex gap-3 flex-wrap mb-10 relative z-10">
                      <span className="px-5 py-2 bg-zinc-50 rounded-2xl text-[9px] font-bold uppercase tracking-[0.2em] text-zinc-400 border border-zinc-100 shadow-sm transition-all group-hover:bg-white group-hover:border-zinc-200">{node.role}</span>
                      <span className="px-5 py-2 bg-zinc-50 rounded-2xl text-[9px] font-bold uppercase tracking-[0.2em] text-zinc-400 border border-zinc-100 shadow-sm transition-all group-hover:bg-white group-hover:border-zinc-200">{node.jobType || 'Remote_Node'}</span>
                    </div>

                    <div className="grid grid-cols-2 gap-4 relative z-10">
                      {tab === 'active' && (
                        <>
                          <button onClick={() => handleAnalyse(node)} className="col-span-2 flex items-center justify-center gap-4 py-6 bg-zinc-950 text-white rounded-2xl text-[10px] font-bold uppercase tracking-[0.3em] hover:scale-[1.02] active:scale-95 transition-all shadow-2xl shadow-black/20 overflow-hidden relative group/btn">
                          ...
                          <Link href={`/dashboard/messages?recipient=${node._id}`} className="flex items-center justify-center gap-3 py-5 bg-white text-zinc-950 rounded-[1.5rem] text-[9px] font-bold uppercase tracking-widest hover:bg-zinc-50 transition-all border border-zinc-100 shadow-sm">
                            <MessageSquare className="h-4 w-4 text-zinc-300" /> Uplink
                          </Link>
                          <button onClick={() => handleAction(node._id, 'soft-delete')} className="flex items-center justify-center gap-3 py-5 bg-white text-rose-600 rounded-[1.5rem] text-[9px] font-bold uppercase tracking-widest hover:bg-rose-50 transition-all border border-zinc-100 shadow-sm">
                      ...
                      {tab === 'pending' && (
                        <>
                          <button onClick={() => handleAction(node._id, 'approve')} className="flex items-center justify-center gap-3 py-6 bg-zinc-950 text-white rounded-2xl text-[10px] font-bold uppercase tracking-widest hover:scale-[1.02] transition-all shadow-2xl shadow-black/20">
                            <Check className="h-5 w-5" /> Authorize
                          </button>
                          <button onClick={() => handleAction(node._id, 'reject')} className="flex items-center justify-center gap-3 py-6 bg-white text-rose-600 rounded-2xl text-[10px] font-bold uppercase tracking-widest border border-zinc-100 hover:bg-rose-50 transition-all shadow-sm">
                      ...
                      {tab === 'blocked' && (
                        <button onClick={() => handleAction(node._id, 'unblock')} className="col-span-2 flex items-center justify-center gap-4 py-6 bg-zinc-950 text-white rounded-2xl text-[10px] font-bold uppercase tracking-widest hover:scale-[1.02] transition-all shadow-2xl shadow-black/20">
                      ...
                      {tab === 'trash' && (
                        <>
                          <button onClick={() => handleAction(node._id, 'restore')} className="flex items-center justify-center gap-3 py-6 bg-white text-zinc-950 rounded-2xl text-[10px] font-bold uppercase tracking-widest border border-zinc-100 hover:bg-zinc-50 transition-all shadow-sm">
                            <RotateCcw className="h-5 w-5" /> Re-Active
                          </button>
                          <button onClick={() => handleAction(node._id, 'hard-delete')} className="flex items-center justify-center gap-3 py-6 bg-rose-600 text-white rounded-2xl text-[10px] font-bold uppercase tracking-widest hover:bg-rose-700 transition-all shadow-xl shadow-rose-900/20">
                ...
                <div className="col-span-full flex flex-col items-center justify-center py-48 text-center bg-zinc-50/50 rounded-3xl border-2 border-dashed border-zinc-100 relative group overflow-hidden">
                  ...
                  <div className="space-y-4 relative z-10">
                    <h3 className="text-3xl font-bold text-zinc-300 uppercase tracking-tighter italic">Sector Empty</h3>
                    <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-[0.5em] italic">Scanning perimeter frequency... 0 records detected</p>
                  </div>
                </div>
              )}
            </motion.div>
        ...
        {analysisNode && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6 sm:p-10">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-5xl bg-white rounded-3xl border border-white/20 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col xl:flex-row"
            >
              <button onClick={() => setAnalysisNode(null)} className="absolute top-10 right-10 p-4 hover:bg-zinc-100 rounded-2xl transition-all text-zinc-400 hover:text-black z-50">
                <X className="h-8 w-8" />
              </button>
              ...
              <div className="xl:w-2/5 p-16 bg-zinc-50 border-r border-zinc-100 flex flex-col items-center text-center relative overflow-hidden">
                ...
                <div className="w-48 h-48 bg-zinc-950 text-white flex items-center justify-center text-7xl font-bold rounded-[3.5rem] mb-12 shadow-2xl relative z-10 group hover:rotate-12 transition-transform duration-700">
                  {analysisNode.name?.[0]?.toUpperCase()}
                </div>
                <div className="space-y-4 relative z-10">
                  <h2 className="text-4xl font-bold text-zinc-950 tracking-tighter uppercase italic">{analysisNode.name}</h2>
                  <div className="flex flex-col items-center gap-2">
                    <p className="text-[11px] font-bold text-zinc-400 uppercase tracking-[0.4em]">{analysisNode.role} Node</p>
                ...
                <div className="mt-16 w-full space-y-6 relative z-10 pt-16 border-t border-zinc-100">
                  <div className="flex items-center gap-6 p-6 bg-white rounded-3xl border border-zinc-100 shadow-sm group hover:border-zinc-950 transition-all">
                    ...
                    <span className="text-[11px] font-bold text-zinc-600 truncate uppercase tracking-widest">{analysisNode.email}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 p-16 bg-white relative overflow-y-auto max-h-[85vh] custom-scrollbar">
                ...
                {analyzing ? (
                  <div className="h-full flex flex-col items-center justify-center py-20 text-center">
                    ...
                    <p className="text-[12px] font-bold uppercase tracking-[0.5em] text-zinc-300 italic">Intercepting Node History Feed...</p>
                  </div>
                ) : analysisData ? (
                  <div className="space-y-16 relative z-10">
                    <div className="bg-zinc-950 rounded-[3rem] p-12 text-white flex flex-col sm:flex-row items-center justify-between gap-10 shadow-2xl shadow-black/30 relative overflow-hidden group">
                      ...
                      <div>
                        <p className="text-[11px] font-bold uppercase tracking-[0.4em] mb-6 text-zinc-500">Node Performance Index</p>
                        <div className="flex items-baseline gap-4">
                          <h4 className="text-9xl font-bold italic tracking-tighter">{analysisData.stats.score}</h4>
                          <span className="text-2xl font-bold text-zinc-700 italic">/100</span>
                        </div>
                      </div>
                      <div className="w-32 h-32 bg-white/5 border border-white/10 rounded-3xl flex items-center justify-center group-hover:scale-110 transition-transform duration-700">
                        <Activity className="h-16 w-16 text-emerald-500 animate-pulse" />
                      </div>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                      <div className="p-10 bg-zinc-50 rounded-3xl border border-zinc-100 shadow-sm group hover:bg-white hover:shadow-xl transition-all duration-500">
                        <p className="text-[10px] font-bold uppercase text-zinc-400 mb-6 tracking-[0.4em]">Operational Efficiency</p>
                        <div className="flex items-baseline gap-4">
                          <span className="text-5xl font-bold text-zinc-950 italic">{analysisData.stats.taskRate}%</span>
                          <span className="text-[11px] font-bold text-zinc-400 uppercase tracking-widest">({analysisData.stats.doneTasks}/{analysisData.stats.totalTasks} Pkts)</span>
                        </div>
                      </div>
                      <div className="p-10 bg-zinc-50 rounded-3xl border border-zinc-100 shadow-sm group hover:bg-white hover:shadow-xl transition-all duration-500">
                        <p className="text-[10px] font-bold uppercase text-zinc-400 mb-6 tracking-[0.4em]">Intelligence Output</p>
                        <div className="flex items-baseline gap-4">
                          <span className="text-5xl font-bold text-zinc-950 italic">{analysisData.stats.totalReports}</span>
                          <span className="text-[11px] font-bold text-zinc-400 uppercase tracking-widest">Broadcasts</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-10">
                      <div className="flex items-center gap-6">
                        <h5 className="text-[12px] font-bold uppercase tracking-[0.5em] text-zinc-950 italic flex items-center gap-4">
                      ...
                      <div className="space-y-4">
                        {analysisData.reports.slice(0, 5).map((r: { _id: string; title?: string; status: string; date: string }) => (
                          <div key={r._id} className="p-8 bg-white border border-zinc-100 rounded-3xl flex flex-col sm:flex-row justify-between items-center gap-6 group/item hover:border-zinc-950 transition-all shadow-sm">
                            <div className="flex items-center gap-6">
                              ...
                              <div>
                                <p className="text-sm font-bold text-zinc-950 uppercase italic tracking-tight">{r.title || 'Standard Operation'}</p>
                        ...
                        {analysisData.reports.length === 0 && (
                          <div className="py-20 text-center bg-zinc-50/50 rounded-[3rem] border border-zinc-100 border-dashed">
                            <p className="text-[10px] font-bold text-zinc-300 uppercase tracking-[0.5em] italic">No active data packets in stream</p>
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="h-full flex flex-col items-center justify-center py-32 text-center">
                    ...
                    <p className="text-[12px] font-bold uppercase tracking-[0.6em] text-zinc-300 italic">Neural Uplink Interrupted: Data Fragment Missing</p>
                  </div>
                )}
              </div>
            </motion.div>
